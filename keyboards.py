...
main_keyboard = ReplyKeyboardMarkup(True)
main_keyboard.row('📋Розклад пар')
main_keyboard.row('⏳Розклад дзвінків', '📆Розклад парних/непарних тижнів')
main_keyboard.row('📔Методички', '🗣 Бесіда', '👨🏻‍🏫👩🏻‍🏫 Викладачі')
main_keyboard.row('🎓Корисні посилання')
main_keyboard.row('👤 Власник бота')

back_keyboard = ReplyKeyboardMarkup(True)
back_keyboard.row('На початок')

shifts_keyboard = ReplyKeyboardMarkup(True)
shifts_keyboard.row('I зміна(1-2 курси)', 'II зміна(3-4 курси)')
shifts_keyboard.row('На початок')

faculty_keyboard = ReplyKeyboardMarkup(True)
faculty_keyboard.row('ОК', 'ЗІ', 'ТО')
faculty_keyboard.row('На початок')

# Groups
ok_groups_keyboard = ReplyKeyboardMarkup(True, True)
ok_groups_keyboard.row('ОК-11', 'ОК-12', 'ОК-13')
...
zi_groups_keyboard = ReplyKeyboardMarkup(True, True)
zi_groups_keyboard.row('ЗІ-11', 'ЗІ-12', 'ЗІ-110')
zi_groups_keyboard.row('ЗІ-21', 'ЗІ-22', 'ЗІ-23')
zi_groups_keyboard.row('ЗІ-31', 'ЗІ-32')
zi_groups_keyboard.row('ЗІ-41', 'ЗІ-42')
zi_groups_keyboard.row('Список відділень', 'На початок')

to_groups_keyboard = ReplyKeyboardMarkup(True, True)
to_groups_keyboard.row('ТО-11', 'ТО-12')
to_groups_keyboard.row('ТО-21', 'ТО-22', 'ТО-110')
to_groups_keyboard.row('ТО-31')
...
monday = InlineKeyboardButton(text="Пн", callback_data='mon')
tuesday = InlineKeyboardButton(text="Вт", callback_data='tue')
wednesday = InlineKeyboardButton(text="Ср", callback_data='wed')
thursday = InlineKeyboardButton(text="Чт", callback_data='thu')
friday = InlineKeyboardButton(text="Пт", callback_data='fri')
days_keyboard.add(monday, tuesday, wednesday, thursday, friday)


links_keyboard = InlineKeyboardMarkup(row_width=2)
our_site = InlineKeyboardButton(text='🌐 Наш сайт', url='https://www.ktkt.com.ua/')
news = InlineKeyboardButton(text='📰 Новини', url='https://www.ktkt.com.ua/?page_id=1219')
scholarship = InlineKeyboardButton(text='💸 Стипендія', url='https://www.ktkt.com.ua/?page_id=2717')
photos = InlineKeyboardButton(text='📸 Фото', url='https://www.ktkt.com.ua/?page_id=430')
youtube = InlineKeyboardButton(text='📺 YouTube', url='https://www.youtube.com/channel/UCOd1kVIz5AoAhgv-7fnb1Gg')
instagram_main = InlineKeyboardButton(text='📱 Instagram', url='https://www.instagram.com/ktktnulp/')
instagram_topktkt = InlineKeyboardButton(text='🔝 Топ КТКТ (Instagram)', url='https://www.instagram.com/top_ktkt/')
links_keyboard.add(our_site, news, scholarship, photos, youtube, instagram_topktkt, instagram_main)


# Для методичок:
faculty_techniques = ReplyKeyboardMarkup(True)
faculty_techniques.row('OK', 'ЗI', 'ТO')
faculty_techniques.row('На початок')

discipline_ok = ReplyKeyboardMarkup(True)
...
discipline_zi = ReplyKeyboardMarkup(True)
...
discipline_to = ReplyKeyboardMarkup(True)
...
ok_files_id = {
    ...
    'Інженерна та комп\'ютерна графіка': 'BQACAgIAAxkDAAJg7V5mm0JD-tjLQKDKwM15_HmvPj6iAALgBQACDcwwSwb1UHrVDrCjGAQ',
